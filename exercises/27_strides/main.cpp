...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    auto stride_r_it = strides.rbegin();
    auto shape_r_it = shape.rbegin();
    *stride_r_it = 1;
    ++stride_r_it;
    ++shape_r_it;
    udim current_stride = 1;
    for (; shape_r_it != shape.rend(); ++shape_r_it, ++stride_r_it) {
        current_stride *= *std::prev(shape_r_it);
        *stride_r_it = current_stride;
    }
    return strides;
}
